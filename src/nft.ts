...
export { getMetadata } from './utils/nft';
 ...
 * @param metaJSON The NFT metadata
 * @param nftMint Keypair of the nft which will be minted
 * @returns
 */
...
export const getBurnTxAndSigner = async (
  ...
  const burnNFTTx = await getNFTBurnTxs(connection, owner.publicKey, mint);
  return {
    txs: [burnNFTTx],
    signers: [payer, owner],
  };
...
export const burn = async (
  ...
  const encodedTx = await batchBurn(
    connection,
    [
      {
        payer,
...
export const updateMetadata = async (
  ...
  const tx = getUpdateMetadataTx(author.publicKey, nftAddress, metaJSON);
  ...
  console.log(`NFT update metadata: signature=${encodedTx.encodedSignature}`);
