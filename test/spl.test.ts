...
describe('SPL TEST', () => {
  ...
  test('mint should be disabled', async () => {
    ...
    const mintInfo = await getMint(connection, mint1_address);
    // console.log(mintInfo);
    expect(mintInfo.supply).toEqual(BigInt(2) * supply);
  });
  test('transfer', async () => {
    const amount = BigInt(new BN('1000').mul(new BN('1000000000')).toString());
  ...
  test('get all token balance', async () => {
    ...
    expect(allInfo[mint1_address.toBase58()]).toEqual(
      BigInt(new BN('1000').mul(new BN('1000000000')).toString()),
    );
  });
  test('burn', async () => {
    const amount = BigInt(new BN('1000').mul(new BN('1000000000')).toString());
