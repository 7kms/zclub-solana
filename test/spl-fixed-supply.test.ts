...
describe('SPL TEST', () => {
  ...
  test('disable mint1 authority', async () => {
    const encodedTx = await SPL.disableMintTokenAuthority(
      connection,
      keypair1,
      keypair1,
      mint1_address,
    ...
    expect(mintInfo.supply).toEqual(supply);
    expect(mintInfo.decimals).toEqual(9);
    expect(mintInfo.mintAuthority).toEqual(null);
  });
  test('mint should be disabled', async () => {
    try {
      const encodedTx = await SPL.mint(
        connection,
        keypair1,
      ...
      expect(true).toEqual(false); // should not be performed
    } catch (e) {
      const t = () => {
        throw e;
      };
      expect(t).toThrowError(/Transaction simulation failed/);
    }
  });
  test('transfer', async () => {
  ...
  test('get all token balance', async () => {
