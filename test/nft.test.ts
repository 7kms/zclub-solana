...
describe('NFT TEST', () => {
  ...
  let mint_chair_address_batch_test7: Web3.PublicKey;
  ...
  beforeAll(async () => {
    ...
    await requestAirdrop(connection, keypair2.publicKey, 1);
  });
  ...
  test('mint collection NFT', async () => {
    ...
    collection_chair_address = await mintANFT(connection, keypair1, metadata);

    const metadata2 = await NFT.getMetadata(
      connection,
      collection_chair_address,
    );
    expect(metadata2.mint).toEqual(collection_chair_address.toBase58());
  ...
  test('update metadata', async () => {
    const metadata = generateMetadata(keypair1.publicKey.toBase58());
    metadata.uri = 'http://123';
    const encodedTx = await NFT.updateMetadata(
      connection,
      keypair1,
      keypair1,
      collection_chair_address,
    ...
    expect(metadata2.data.uri).toEqual(expect.stringMatching(metadata.uri));
  });
  ...
  test('mint chairs 2-7', async () => {
    ...
    mint_chair_address_batch_test7 = await mintANFT(
      connection,
      keypair1,
      generateMetadata(
        keypair1.publicKey.toBase58(),
  ...
  test('batch transfer', async () => {
    const encodedTx = await NFT.batchTransfer(connection, [
      ...
      },
      {
        ...
        source: keypair1,
        destination: keypair2.publicKey,
        mint: mint_chair_address_batch_test7,
      },
    ]);
    await connection.confirmTransaction(
      await connection.sendEncodedTransaction(encodedTx.encodedSignature),
    ...
    expect(allTokenInfo[mint_chair_address_batch_test7.toBase58()]).toEqual(
      BigInt(1),
    );
  });
  test('burn', async () => {
